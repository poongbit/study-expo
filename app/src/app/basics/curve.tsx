...
import { curveGuideForIndex, CURVE_GUIDES } from '../../basics/curveExercises';
import { createCurveAttempt, loadCurveAttempts, saveCurveAttempt } from '../../basics/curveStore';
import { analyzeCurveBatch, CURVE_METRIC_LABELS, CURVE_TARGETS, type CurveAttempt, type CurveMetricKey } from '../../basics/analyzeCurve';
...
export default function CurveScreen() {
  ...
  const [history, setHistory] = useState<CurveAttempt[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState(false);
  const [strokes, setStrokes] = useState<PracticeStroke[]>([]);
  const [active, setActive] = useState(false);
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<CurveAttempt | null>(null);
  ...
  const readHistory = useCallback(async () => {
    ...
    try {
      const data = await loadCurveAttempts();
  ...
  const batch = useMemo(() => setAttempts.length >= SET_SIZE ? analyzeCurveBatch(setAttempts) : null, [setAttempts]);
  ...
  useEffect(() => {
    ...
    const rx = problem ? prescriptionFor('curve', problem) : null;
    const metricValues = batch.metrics;
    const evidence = problem
      ? `${CURVE_METRIC_LABELS[problem]}: 평균 ${(metricValues[problem].mean * 100).toFixed(1)}%, 초과 빈도 ${(metricValues[problem].exceededFrequency * 100).toFixed(0)}%`
      : '특별히 반복된 문제 없음';
  ...
  const scheduledGuide = curveGuideForIndex(currentIndex);
  ...
  const analyze = async () => {
    ...
    try {
      const attempt = result ?? createCurveAttempt(strokes, previous as CurveAttempt | null, scheduledGuide, { practiceSetId: setId, setRuleVersion: 1 });
      setResult(attempt);
      await saveCurveAttempt(attempt);
  ...
  const renderCanvas = () => {
    if (isResultPhase) {
      // For curves, we just show a representative empty canvas or the last drawn curve
      return (
    ...
    return (
      <View style={styles.panelScroll}>
        ...
        <Text style={[styles.panelDesc, { color: c.textSecondary }]}>
          한 획을 기록하면 캔버스를 비우고 다음 커브를 안내합니다. 20획을 모두 모은 뒤 종합 피드백을 알려드려요.
        </Text>
